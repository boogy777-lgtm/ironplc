...
class ProblemIndex(Directive):
    """Emit a ``toctree`` of every problem-code page in the current directory.

    Usage (in a problems ``index.rst``)::

        .. problem-index:: V

    The single argument is the code prefix (``P``, ``E`` or ``V``). The list of
    pages is computed at parse time from the sibling ``<prefix>####.rst`` files
    and sorted by numeric code, so the committed source stays a stable stub and
    never produces merge conflicts when problem codes are added.
    """
    required_arguments = 1
    ...
    def run(self):
        ...
        docdir = PurePosixPath(env.docname).parent
        srcdir = Path(env.doc2path(env.docname)).parent

        pattern = re.compile(rf'{re.escape(prefix)}\d+$')
        stems = [
            path.stem
            for path in srcdir.glob(f'{prefix}*.rst')
            if pattern.match(path.stem)
        ]
        stems.sort(key=lambda stem: int(re.match(rf'{re.escape(prefix)}(\d+)', stem).group(1)))

        docnames = [str(docdir / stem) for stem in stems]

        subnode = addnodes.toctree()
        subnode['parent'] = env.docname
        subnode['entries'] = [(None, docname) for docname in docnames]
        subnode['includefiles'] = docnames
        subnode['maxdepth'] = 1
        subnode['caption'] = None
        subnode['glob'] = False
        subnode['hidden'] = False
        subnode['includehidden'] = False
        subnode['numbered'] = 0
        subnode['titlesonly'] = False
        subnode.source, subnode.line = self.state_machine.get_source_and_line(self.lineno)
...
def setup(app):
    ...
    app.add_directive("problem-index", ProblemIndex)
