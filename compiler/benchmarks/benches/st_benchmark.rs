...
/// `STRING_TO_<numeric>` under the strict policies (reject, trap) — the
/// scan-and-range-check cost of one conversion, per target width. Each
/// program converts a valid literal one hundred times; the integer widths differ
/// only in the bounds the scanner checks, so the integer cases should sit
