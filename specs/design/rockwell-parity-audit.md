...
- **[HA Architecture Readiness](https://github.com/boogy777-lgtm/ironplc/blob/8a7f6d0d09b00436daebfd669babd39f2e0f2e13/specs/design/ha-architecture-readiness.md)**: the reuse
...
| Session model | One open session per profile; one active connection per workspace (engineering-connection.md, invariants, Out of Scope); one host per served process (`compiler/vm-cli/src/serve.rs:31`, MCP per ADR-0056); the readiness doc records "the session model is single-host" ([historical readiness analysis](https://github.com/boogy777-lgtm/ironplc/blob/8a7f6d0d09b00436daebfd669babd39f2e0f2e13/specs/design/ha-architecture-readiness.md), Engineering backend surface) | Rockwell controllers serve many concurrent online workstations | TCP listener (Mechanism 2 framing is already designed) accepting N sessions; a session registry; read-only observer sessions beside one editor session | M |
