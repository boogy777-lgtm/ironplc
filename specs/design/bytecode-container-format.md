...
Type-changing migration is policy-gated ([ADR-0060](../adrs/0060-type-changing-migration-policies.md)): when a shared UID's variable-table entry differs only in `var_type`, the planner consults a fixed table of admitted storage-class conversions — integer widening within the signed and unsigned families (`I32` -> `I64`, `U32` -> `U64`), signed integer to real (`I32` -> `F32`, `I32` -> `F64`, `I64` -> `F64`), and real widening (`F32` -> `F64`). An admitted scalar plans a conversion that executes at the scan boundary; an admitted array element pair converts element-wise, but only when the element count and the per-element size are equal. Strings migrate only within the same width and maximum length (a same-size copy); every other pair — narrowing, signedness changes, `TIME`, strings of a different size, FB instances — rejects the candidate at stage time with the pair named, through the same `MigrationUnsupported` (V4010) path as any other unprovable migration. Within-class widenings such as `INT` -> `DINT` change no `VarEntry` at all (both encode as `I32`), so the hash stays equal and no migration is involved.
