...
### 15.4. Общий механизм engineering: синтез Rockwell и Schneider

**Инженер задаёт намерение → владелец операции проверяет условия → выполняет переход → публикует результат. IDE показывает факты и доступные команды.**

Это архитектурный синтез для IronPLC, а не описание внутренней реализации Rockwell или Schneider. Промышленные референсы и границы переноса приведены в §24.1. Общая форма взаимодействия дополняет контракты §6/15.2; она не создаёт единого менеджера или одной глобальной FSM. Владелец операции обращается к владельцам ресурсов по их контрактам: например, DeploymentCoordinator ведёт activation operation, а RuntimeHost единолично переключает ExecutionBinding (§10.1).

Схема показывает путь команды, а не полный lifecycle операции. Выполнение может закончиться Failed, Cancelled или UnknownOutcome; Accepted, Applied и Durable различаются по §6. Публикация результата означает доступность наблюдения через API, а не обязательное получение ответа подключённой IDE.

```mermaid
flowchart TD
    A["IDE: проект и намерение"] --> B["Команда с целевой ревизией"]
    B --> C{"Владелец операции: допуск"}
    C -->|Разрешено| D["Выполнение и переход состояния"]
    C -->|Запрещено| E["Отказ с причиной"]
    D --> F["Факты, ревизии и результат"]
    E --> F
    F --> G["IDE: статусы и доступные команды"]
    F --> C
```

#### 1. У каждого факта один владелец

| Область | Что хранит | Владелец / контракт |
|---|---|---|
| Engineering workspace | Исходники, локальные правки, результат сборки | Инженерный проект на workstation; не authoritative state PLC |
| Engineering session | Соединение, идентичность PLC, контекст согласования проекта | EngineeringService / client session; actual PLC facts читаются у их владельцев |
| Deployment | Подготовленный кандидат, trial, finalize/revert | DeploymentCoordinator; immutable artifacts и pins принадлежат ApplicationCatalog |
| Execution | Фактическое исполнение и выбранная generation | RuntimeHost / ExecutionSession |
| HA | PRIMARY/SECONDARY, квалификация и готовность резерва | RoleCoordinator ведёт role/handover; ReplicationEngine — checkpoint/ACK facts; readiness — проекция, OutputAuthority отдельно обеспечивает право на outputs |
| Firmware update | Ход maintenance/update/recovery | FirmwareUpdater и secure loader по §13 |

Общий принцип переходов не требует единого менеджера или одной гигантской FSM. Локальная правка, состояние исполнения, соответствие проектов и пригодность резерва относятся к разным владельцам и могут изменяться независимо в пределах межкомпонентных инвариантов.

#### 2. Статусы вычисляются из фактов

Соответствие двух артефактов определяется в явно выбранном scope:
...
Здесь Valid означает пригодность наблюдения для данного сравнения: идентичность известна и проверена, scope и схема идентичности совместимы, provenance/revision/freshness удовлетворяют контракту. Это не только синтаксическая валидность файла. Identity относится к явно выбранному предмету сравнения: executable, schema, configuration или целому manifest; имя файла, адрес памяти и размер не заменяют identity.

| Отношение | Что узнаём |
|---|---|
| Собранный проект ↔ активное приложение PLC | Соответствует ли инженерный артефакт исполняемому |
| PRIMARY ↔ SECONDARY | Совпадают ли требуемые артефакты пары |
| Активное приложение ↔ boot image | Совпадает ли активный артефакт с выбранным для восстановления после перезапуска |

Один механизм сравнения применяется к разным отношениям. Результат содержит стороны, scope, identities/revisions и freshness. После утраты актуальности live observation прежний EQUAL не выдаётся за текущее подтверждённое соответствие; историческое значение может отображаться отдельно как stale.

**EQUAL не означает RUN, а совпадение приложений пары не означает готовность к takeover.** Match артефактов не подтверждает равенство текущих значений переменных, совместимость checkpoint или наличие output authority. Сравнение с boot image также не заменяет проверку durable receipts и reset/retain policy (§12/13).

Соединение с PLC и согласование инженерного проекта различаются: доступность controller API не доказывает соответствие открытого проекта активному приложению. Correlation использует сравнение и разрешённые операции согласования; сам EQUAL/DIFFERENT остаётся вычисляемым отношением, а не вторым владельцем приложения.

#### 3. Команда допускается по сочетанию условий

Общая форма:

$$
Allow(cmd)=Authorized
\land RevisionCurrent
\land StateAllows
\land PreconditionsMet
$$

| Условие | Смысл |
|---|---|
| Authorized | Principal имеет необходимые capabilities в целевом scope; действуют ограничения ключа и security policy |
| RevisionCurrent | ControllerId/BootId и ожидаемые revisions относятся к действующему target/context; запрос не основан на устаревшем состоянии |
| StateAllows | Текущая фаза операции и режим ресурса допускают команду; конфликтующие операции согласованы |
| PreconditionsMet | Выполнены конкретные compatibility, resource, timing, state и HA/authority условия команды |

Конкретные условия определяет владелец команды. Для Test Edits это готовность кандидата, совместимость состояния, режим CPU и применимые условия HA (§9–11). Для firmware update — собственные maintenance, trust, bootloader и compatibility условия (§13).

Проверка в IDE помогает инженеру; **окончательный допуск и повторная проверка перед commit принадлежат контроллеру**. Разрешённая кнопка не является полномочием. Изменение условий между admission и commit приводит к предусмотренному контрактом отказу/reconciliation, а не к исполнению по старому разрешению. Команда несёт OperationId, target, expected revisions, principal, deadline и digest согласно §6/15.2; физические эффекты дополнительно проходят §8.4/11.2.

#### 4. Hot edit использует этот механизм

| Действие | Результат |
|---|---|
| Редактирование | Меняется локальный workspace |
| Stage / Accept | Кандидат подготовлен после успешных verify/prepare; Original продолжает исполняться |
| Test | На допустимой границе выбирается кандидат; право на новые физические effects отдельно проверяется по §11.5 |
| Untest / Revert | Выполняется проверенный возврат к предыдущему коду с текущим совместимым state или допустимой обратной migration |
| Finalize | Закрепляется выбранная generation по transaction/durability contract §10 |
| Discard | Удаляется неисполняемый кандидат после освобождения его references/pins |

Stage / Accept здесь — инженерное обозначение пути подготовки; наличие артефакта в ApplicationCatalog само по себе не означает PREPARED. Фазы RECEIVING/VERIFIED/PREPARED и границы receipts остаются определёнными в §10. Таблица не вводит параллельную hot-edit FSM.

Локальные новые правки могут существовать одновременно с trial на PLC — это состояния разных владельцев. Новые Pending Edits не заменяют уже принятый candidate и не обходят ограничения capacity/pins и сериализации активаций. Trial в RUN воздействует на процесс; Test Edits отличается от TEST mode. Revert возвращает код по проверенной процедуре, а не прошлое физическое состояние процесса.

**Проверка N+1:** новый статус добавляет проекцию фактов; новая однотипная команда — условия и переход существующего lifecycle. Новый независимый механизм нужен при появлении действительно новой ответственности (§21). Различия вендоров выражаются политиками только там, где сохраняются инварианты: например, политика продолжения trial или возврата Original при takeover требует согласованного RecoveryAdmission и state compatibility (§11.5/24.1), а не одного UI-флага.
